...
def format_plan(plan, result=None, dry_run=True):
    lines = [f"# readiness-fix plan{' (dry run — no files written)' if dry_run else ''}", ""]
...
def run_fix(args) -> int:
    ...
    if report is None:
        sys.stderr.write("readiness fix: no report found; run `readiness report` first.\n")
        return 2
    ...
    if worktree_dirty(root) and not getattr(args, "force", False):
        sys.stderr.write("readiness fix: working tree has uncommitted changes; commit/stash or use --force.\n")
        return 1
